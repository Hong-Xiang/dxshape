        ...
        if (axis is None) and (angle is None):
            if rotate_matrix.shape[0] is None:
                ...
                raise ValueError(fmt.format(
                    rotate_matrix.shape[0], self.dim()))
 #           else:
            elif self.dim() == rotate_matrix.shape[0]:
            ...
            rotated_point = np.transpose(np.dot(np.dot(rotation_matrix3, (rotation_matrix2, np.dot(
                rotation_matrix1, np.transpose(np.array(trans_p._data)))))))
